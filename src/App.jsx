...
const Button = ({ children, variant = 'outline', className = '', onClick }) => {
  ...
  const styles = {
    outline: "border border-[#BFA05A] text-[#1C1917] hover:text-white",
    solid: "bg-[#BFA05A] text-white border border-[#BFA05A]",
    ghost: "text-[#1C1917] border-b border-[#1C1917]/20 hover:border-[#BFA05A] px-0 py-2 hover:pl-4"
  };
  ...
  return (
    <button onClick={onClick} className={`${base} ${styles[variant]} ${className}`}>
      ...
      {variant === 'outline' && (
        <div className="absolute inset-0 bg-[#BFA05A] transform scale-x-0 group-hover:scale-x-100 transition-transform duration-700 origin-left ease-out z-0" />
      )}
    </button>
  );
};
  ...
  return (
    <>
      <nav className={`fixed w-full z-50 transition-all duration-700 ${
        scrolled ? 'py-4 bg-[#F9F8F6]/80 backdrop-blur-xl border-b border-[#1C1917]/5' : 'py-6 bg-transparent'
      }`}>
        <div className="container mx-auto px-6 md:px-12 flex justify-between items-center">
          ...
          <div className="hidden md:flex items-center gap-12">
            {['Filosofía', 'Rituales', 'Ubicación'].map((item) => (
              <a 
                ...
                className={`text-xs uppercase tracking-[0.2em] font-sans hover:text-[#BFA05A] transition-colors duration-500 ${
                  scrolled ? 'text-[#1C1917]' : 'text-white/90'
                }`}
              >
                {item}
              </a>
            ...
            <button 
              ...
              className={`px-6 py-2 border text-xs uppercase tracking-[0.2em] transition-all duration-500 hover:bg-[#BFA05A] hover:border-[#BFA05A] hover:text-white ${
                scrolled ? 'border-[#1C1917] text-[#1C1917]' : 'border-white text-white'
              }`}
            >
              Reservar
            </button>
          ...
          <button 
            className="md:hidden z-50 text-[#BFA05A]"
            onClick={() => setMenuOpen(!menuOpen)}
          >
            {menuOpen ? <X size={24} /> : <Menu size={24} color={scrolled ? '#1C1917' : 'white'} />}
          </button>
        </div>
      </nav>
      ...
      <AnimatePresence>
        {menuOpen && (
          <motion.div 
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 bg-[#F9F8F6] z-40 flex items-center justify-center"
          >
    ...
    <section className="relative h-screen w-full overflow-hidden">
      ...
      <div className="relative z-20 h-full flex flex-col justify-center items-center text-center px-6">
        <motion.div
          ...
          className="space-y-8"
        >
          <p className="text-[#BFA05A] font-sans text-xs md:text-sm tracking-[0.4em] uppercase">
          ...
          <div className="pt-12">
            <button 
              ...
              className="group relative px-12 py-4 border border-white/30 text-white overflow-hidden transition-all duration-700 hover:border-[#BFA05A]"
            >
              <span className="relative z-10 text-xs tracking-[0.3em] uppercase group-hover:text-white transition-colors duration-500">
                Explorar Rituales
              </span>
              <div className="absolute inset-0 bg-[#BFA05A] transform scale-y-0 group-hover:scale-y-100 transition-transform duration-500 origin-bottom z-0 opacity-80" />
            </button>
          </div>
        </motion.div>
      </div>
...
const ServiceItem = ({ service, index }) => {
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-[80vh] w-full">
      ...
      <div className={`w-full md:w-1/2 bg-[#F9F8F6] flex flex-col justify-center p-12 md:p-24 lg:p-32 ${isEven ? 'md:order-2' : 'md:order-1'}`}>
        <motion.div
        ...
        >
          <div className="flex items-center gap-4 mb-6">
            <Minus className="text-[#BFA05A]" size={40} strokeWidth={1} />
...
const Services = () => {
  return (
    <section id="rituales" className="bg-[#F9F8F6]">
      <div className="py-32 container mx-auto px-6 text-center">
        <span className="text-[#BFA05A] text-xs tracking-[0.4em] uppercase block mb-6">
            Menú de Servicios
        </span>
    ...
    <section className="relative h-[80vh] flex items-center justify-center overflow-hidden">
      ...
      <div className="relative z-10 max-w-4xl px-6 text-center">
        ...
        >
          ...
          <h2 className="font-serif text-4xl md:text-6xl lg:text-7xl text-[#F9F8F6] leading-tight italic">
            "Desconecta para <br/> reconectar con tu esencia"
          </h2>
          <div className="w-24 h-[1px] bg-[#BFA05A] mx-auto mt-12" />
        </motion.div>
      </div>
    </section>
  );
...
const Footer = () => {
  return (
    <footer id="ubicación" className="bg-[#11100F] text-[#F9F8F6] py-24 border-t border-white/5">
      <div className="container mx-auto px-6 md:px-12">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-16 lg:gap-24">
          <div className="space-y-8">
            <h3 className="font-serif text-3xl tracking-widest text-white">SPA VIVIAN</h3>
            ...
            <div className="flex gap-4">
              <a href={SOCIAL.instagram} target="_blank" rel="noreferrer" className="text-white/40 hover:text-[#BFA05A] transition-colors duration-500">
                <Instagram size={20} />
              </a>
              <a href={SOCIAL.whatsapp} target="_blank" rel="noreferrer" className="text-white/40 hover:text-[#BFA05A] transition-colors duration-500">
                <Phone size={20} />
              </a>
            </div>
          </div>
          ...
          <div className="space-y-8">
            <h4 className="font-sans text-xs tracking-[0.2em] uppercase text-[#BFA05A]">Visítanos</h4>
            <div className="space-y-4">
              ...
              <div className="flex gap-4 items-start">
                <div className="w-4 h-4 mt-1 rounded-full border border-white/20 flex items-center justify-center shrink-0">
                  <div className="w-1 h-1 bg-[#BFA05A] rounded-full"></div>
                </div>
...
const App = () => {
  return (
    <div className="min-h-screen bg-[#F9F8F6] selection:bg-[#1C1917] selection:text-[#BFA05A]">
      <style>{`
